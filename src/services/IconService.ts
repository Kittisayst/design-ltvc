...
export const IconService = {
     ...
     * Search for icons.
     */
    searchIcons: async (query: string, limit: number = 50): Promise<string[]> => {
        try {
     ...
     */
    getIconSVG: async (iconName: string): Promise<string | null> => {
        try {
     ...
     */
    getIconPreviewUrl: (iconName: string): string => {
        const parts = iconName.split(':');
